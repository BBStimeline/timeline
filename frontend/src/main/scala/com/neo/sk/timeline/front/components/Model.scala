...
//class Model(title:String,body:Node,content:String,successFun:()=>Unit){
//class Model(title:String,modleBody:Elem,confirmStr:String,successFun:()=>Unit,bottomtype:Int=0,modelBottom:Elem){
class Model(title:String,modleBody:Elem,confirmStr:String,minheight:Int,minwidth:Int,successFun:()=>Unit){
  //  val modelDom=if(bottomtype==0){
  val modelDom=
    <div id="modledom" class="model" >
      <div class="modelbox" style={s"min-height:${minheight}px;min-width:${minwidth}px;"}>
        ...
        <div class="modelbottom">
          <button class="modelbutton" style="color: #FFFFFF;border: 0px ;background-color:#4D78FB;" onclick={() => {successFun()} } >{s"$confirmStr"}</button>
...
object Model {
  ...
  def apply(title: String, modleBody: Elem, confirmStr: String, minheight: Int, minwidth: Int, successFun: () => Unit): Model = new Model(title, modleBody, confirmStr, minheight, minwidth, successFun)
}
