...
/**
  ...
  * Time: 11:21
  */
object AdminLoginPage extends Index {
  ...
  def login():Unit ={
    ...
    if(!name.equals("") && !password.equals("")){
      ...
      Http.postJsonAndParse[SuccessRsp](Routes.AdminRoutes.login,bodyStr).map{
        case Right(rsp) =>
          if(rsp.errCode!=0){
  ...
  override def render:Elem  = {
    <div class="login-background" style="width: 100%;height: 100%;
    text-align: center;
    font-size: x-large;
    font-style: italic;
