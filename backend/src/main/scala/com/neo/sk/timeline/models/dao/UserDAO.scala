...
object UserDAO {
  ...
  def cleanUserFeed(uid: Long, authorId: Long, origin:Int) = db.run{
    tUserFeed.filter(i => i.userId === uid && i.authorId === authorId &&i.origin===origin&& i.feedType === FeedType.USER).delete
  }
